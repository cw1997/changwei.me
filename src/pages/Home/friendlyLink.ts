export interface IFriendlyLinkItem {
  name: string
  url: string
  iconUrl?: string
}

const friendlyLink: IFriendlyLinkItem[] = [
  {name: `昌维的博客`, url: `https://blog.changwei.me/`},
  {name: `zhangguanzhang's Blog`, url: `https://zhangguanzhang.github.io/`},
  {name: `猫叔's Blog`, url: `https://www.vicicode.com/`},
  {name: `Ah!'s Blog`, url: `https://blog.forgiveher.cn/`},
  {name: `KeHan Lu`, url: `https://hanklu.tw/`},
  {name: `Hank Learning`, url: `https://blog.hanklu.tw/`},
  {name: `劣質文章農場`, url: `https://blog.splitline.tw/`},
  {name: `小青青的秘密基地`, url: `http://largeq.cn/`},
  {name: `Leo's Blog`, url: `https://freeleox.github.io/`},
  {name: `Cytosineの有被小窝~`, url: `https://cyto.top/`},
  {name: `小狐素涵`, url: `http://lolitaice.com`},
  {name: `冰柠配绿茶的秘密后花园`, url: `https://frozenlemontee.top`},
  {name: `杨泽宇的主页`, url: `https://processon.com/u/60940b717d9c084071aeb1b0/profile#pc`},
]

export default friendlyLink
